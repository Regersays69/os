...
echo -e "$green                $NC"                                                                                                                          
echo -e "$green♥ TERIMAKSIH TELAH MEMAKAI SCRIPT MANDIRI NETWORK ♥$NC"
sleep 2
sleep 5
echo -e  "${green}┌──────────────────────────────────────────┐${NC}"
...
echo "MANDIRI NETWORK" > /etc/xray/username
echo ""
clear
author=$name
echo ""
...
function key2(){
    ...
    echo -e "${green}└──────────────────────────────────────────┘${NC}"
        ...
        curl -s https://imortall.web.id/token > /etc/github/api
        curl -s https://imortall.web.id/email > /etc/github/email
        curl -s https://imortall.web.id/nama > /etc/github/username
        clear
...
MYIP=$(curl -sS ipv4.icanhazip.com)
IZIN=$(curl -sS https://raw.githubusercontent.com/regersays69/izinvps/main/ip | grep $MYIP | awk '{print $3}' )
d1=$(date -d "$IZIN" +%s)
d2=$(date -d "$today" +%s)
EXP=$(( (d1 - d2) / 86400 ))
