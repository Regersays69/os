...
username=$(curl -sS https://raw.githubusercontent.com/regersays69/izinvps/main/ip | grep $MYIP | awk '{print $2}')
valid=$(curl -sS https://raw.githubusercontent.com/regersays69/izinvps/main/ip | grep $MYIP | awk '{print $3}')
today=$(date +"%Y-%m-%d")
d1=$(date -d "$valid" +%s)
d2=$(date -d "$today" +%s)
...
# URL repository
REPO="https://raw.githubusercontent.com/regersays69/os/main/"
pwadm="@Ridwan112#"
